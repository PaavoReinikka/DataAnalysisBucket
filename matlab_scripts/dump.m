function y = dump(featureInfo, p_benjamini, p_uncorrected, ...
    ...
    if nargin < 8
        assert(false, 'Wrong number of inputs!');
    elseif nargin < 9
        keep_all = true;
    end
    ...
    header = sprintf("ID;Name;masstime" + ...
    ...
    for i=1:n_tests
        if(h(ind(i)) && hh(ind(i)))
            ...
            %fprintf(fid,'%s;',featureInfo.Var5(feat_ind));
            fprintf(fid,'%s@',featureInfo.Var6(feat_ind));%mass
            fprintf(fid,'%s;',featureInfo.Var7(feat_ind));%time
    ...
    %fid_fc = fopen('foldChange.csv','w'); %open file and create fid
    %fid_mmargin = fopen('minMargin.csv','w'); %open file and create fid
